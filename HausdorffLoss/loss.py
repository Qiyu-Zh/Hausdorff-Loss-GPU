...
class HD_ce_loss(nn.Module):
    """Binary Hausdorff loss based on distance transform"""

    def __init__(self, apply_nonlin: Callable = None, alpha=2.0):
        super(HD_ce_loss, self).__init__()
        self.alpha = alpha
        self.apply_nonlin = apply_nonlin
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            target_dt = self.distance_field(target.to(dtype=torch.bool))
        if pred.shape[1] == 1: 
            pred_error = F.binary_cross_entropy_with_logits(pred, target.float(), reduction='none').unsqueeze(1)
        else:
            pred = pred[:, 1:]
            pred_error = torch.zeros_like(pred, dtype=torch.float32, device=pred.device)    
            for c in range(pred.shape[1]):
                pred_error[:, c] = F.binary_cross_entropy_with_logits(pred[:, c], target[:, c].float(), reduction='none')               
        distance = pred_dt ** self.alpha + target_dt ** self.alpha
